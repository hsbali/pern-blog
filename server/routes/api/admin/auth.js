...
router.get("/refresh", (req, res) => {
  try {
    const token = req.cookies.___admin_refresh_token;
    ...
    if (!Number.isInteger(decoded.userId) || !decoded.userType) {
      return res.status(401).json({ status: 0, msg: "Access Denied" });
    }
    ...
    return res
      .cookie("___authenticated_admin", true, {
        path: "/",
        expires: new Date(new Date().getTime() + 168 * 60 * 60 * 1000),
        httpOnly: false,
      })
      .cookie("___admin_refresh_token", refreshToken, {
        path: "/",
        expires: new Date(new Date().getTime() + 168 * 60 * 60 * 1000),
        httpOnly: true,
      })
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({ status: 0, msg: "Admin not found" });
    }

    return res.json({ status: 1, msg: "Admin loaded", data: { user } });
  } catch (err) {
    console.log(err.message);
    return res.status(500).json({ status: 0, msg: "Something went wrong!" });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const getUser = await pool.query(
        "SELECT * FROM tbl_user WHERE email = $1 AND user_type = '1'",
        [email]
      );
      const user = getUser.rows[0];

      if (!user) {
        return res.status(400).json({ status: 0, msg: "Access Denied" });
      }
