...
// @desc        Get All Approved and user comments
// @access      Public
router.get("/posts/:postId", auth(1), async (req, res) => {
  try {
    const getComments = await pool.query(
      "SELECT c.comment_id, c.content, c.url, c.create_time, c.is_approved, u.user_id, u.username, u.email, u.user_type, p.title FROM tbl_comment c JOIN tbl_user u ON c.user_id = u.user_id JOIN tbl_post p ON c.post_id = p.post_id WHERE c.post_id = $3 ORDER BY create_time DESC OFFSET $1 LIMIT $2",
      [
